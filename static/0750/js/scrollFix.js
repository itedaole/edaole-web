;(function($) {
  jQuery.fn.scrollFix = function(height, dir) {
    ...
    var oldScrollHeiht = height;
    return this.each(function() {
      if (height == "bottom") {
        height = document.documentElement.clientHeight - this.scrollHeight;
        oldScrollHeiht = this.scrollHeight;
      } else if (height < 0) {
        height = document.documentElement.clientHeight - this.scrollHeight + height;
        oldScrollHeiht = this.scrollHeight - height;
      }
      var that = $(this),
        oldHeight = false,
      ...
      if (window.XMLHttpRequest) { //非ie6用fixed
        ...
        $(window).resize(function() {
        	if(dir == "bottom"){
        		if (oldHeight === false) {
        			height = document.documentElement.clientHeight - oldScrollHeiht;
        		}
        		else {
        			height = document.documentElement.clientHeight - oldScrollHeiht;
            		that.css({
