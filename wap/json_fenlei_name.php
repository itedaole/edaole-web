...
list($pagesize, $offset, $pagestring) = pagestring($count, 18);
...
$pagetitle = '团购大全';
include template('json_fenlei_name');
...
//			);
	...
	if($group_id=='0' && $ncon!='new') { $a =  ",{\"fenleiurl\":\"/json_fenlei.php?city_id={$city_id}&pid={$pid}&sid=0&oid={$oid}&did={$direction}\",\"fenleiname\":\"所有项目\"}"; }
	else { $a =  ",{\"fenleiurl\":\"/json_fenlei.php?city_id={$city_id}&pid={$pid}&sid=0&oid={$oid}&did={$direction}\",\"fenleiname\":\"所有项目</a> "; }
	if($ncon=='new') { $a .=  ",{\"fenleiurl\":\"/json_fenlei.php?city_id={$city_id}&pid={$pid}&sid=0&oid={$oid}&did={$direction}&ncon=new\"\",\"fenleiname\":\"今日新单\"}"; }
	else { $a .=  ",{\"fenleiurl\":\"/json_fenlei.php?city_id={$city_id}&pid={$pid}&sid=0&oid={$oid}&did={$direction}&ncon=new\",\"fenleiname\":\"今日新单\"}"; }
	...
	foreach($categorys AS $id=>$name) {
		...
		if($id==$group_id) { $a .=  ",{\"fenleiurl\":\"json_fenlei.php?group_id={$id}&city_id={$city_id}&pid={$pid}&sid=0&oid={$oid}&did={$direction}\",\"fenleiname\":\"$name\" }"; }
		else { $a .=  ",{\"fenleiurl\":\"/json_fenlei.php?group_id={$id}&city_id={$city_id}&pid={$pid}&sid=0&oid={$oid}&did={$direction}\",\"fenleiname\":\"$name\" }"; }
	}
