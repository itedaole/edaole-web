...
// 		'/order/index.php' => '我的订单',
		...
		'/account/comment.php' => '我的评价',
		'/account/collect.php' => '我的收藏',
	);
...
function current_link_a($link, $links, $span=false) {
	$html = '';
	$span = $span ? '<span></span>' : '';
	foreach($links AS $l=>$n) {
		if (trim($l,'/')==trim($link,'/')) {
			$html .= "<a class=\"focus\" href=\"{$l}\">{$n}</a>";
		}
...
function current_link_li($link, $links, $span=false) {
	$html = '';
	$span = $span ? '<span></span>' : '';
	foreach($links AS $l=>$n) {
		if (trim($l,'/')==trim($link,'/')) {
			$html .= "<li><a  class=\"focus\" href=\"{$l}\">{$n}</a>{$span}</li>";
		}
		else $html .= "<li><a href=\"{$l}\">{$n}</a>{$span}</li>";
	}
	return $html;
