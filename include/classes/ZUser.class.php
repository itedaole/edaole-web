...
// 		}
	...
	static public function createCouponcode($user_id){
		$couponcode = (rand(1000,9999)).rand(100,999);
		$count = Table::Count('user', array(
				'id <>' => $user_id,
				'couponcode' => $couponcode,
		));
		
		if($count){
			self::createCouponcode($user_id);
		}
		else {
			 $result = self::Modify($user_id,array('couponcode'=>$couponcode));
			 if ($result) {
			 	return $couponcode;
			 }
			 return '';
