...
elseif ( 'categoryedit' == $action ) {
	...
	if ($zone[0] == 'express') {
		...
		$citys=Utility::OptionArray($categorys,'id','name');
		$html = render('manage_ajax_dialog_categoryarea');
	}else{
...
else if ( 'topicremove' == $action ) {
	Table::Delete('xh_topic', $id);
	Session::Set('notice', "专题 {$id} 删除成功！");
	json(null, 'refresh');
}
elseif ( 'billdetail' == $action) {
	 ...
	 $html = render('manage_ajax_dialog_billdetail');
	 json($html, 'dialog');
}
elseif ( 'billedit' == $action) {
     $bill = Table::Fetch('partner_bill', $id);
	 Table::UpdateCache('partner_bill', $id, array(
					'bill_status' => strval($_GET['s']),
					'mnote'=>strval($_GET['t']),
			    ));
	 json( array(
				array('data'=>'审核成功', 'type' => 'alert',),
				array('data'=>'X.boxClose();', 'type' => 'eval',),
				array('data'=>'null', 'type' => 'refresh',),
			   ), 'mix');
}
